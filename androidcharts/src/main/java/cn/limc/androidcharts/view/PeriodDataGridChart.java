...
public abstract class PeriodDataGridChart extends DataGridChart {
    ...
    protected void initAxisY() {
        ...
        if (decimberNumber != 0){
            ...
            for (int i = 0; i < simpleGrid.getLatitudeNum(); i++) {
                String value = decimerValue(minValue + i * average);
                titleY.add(value);
            }
            // calculate last degrees by use max value
            String value = decimerValue(maxValue);
            titleY.add(value);
        }else{
            // calculate degrees on Y axis
            for (int i = 0; i < simpleGrid.getLatitudeNum(); i++) {
    ...
    private String decimerValue(double value){
        double displayValue = value;

        String axisvalue;
        if (this.decimberNumber == 0){
            axisvalue = "";
        }else if (this.decimberNumber == 1){
            axisvalue = ".0";
        }else if (this.decimberNumber == 2){
            axisvalue = ".00";
        }else if (this.decimberNumber == 3){
            axisvalue = ".000";
        }else{
            axisvalue = ".00";
        }
        ...
        if (displayValue < 10000) {
            return new DecimalFormat(axisYDecimalFormat+axisvalue).format(displayValue);
